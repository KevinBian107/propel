...
0. **Selects a mode** — choose how you want to work: **Researcher** (literature + investigation), **Engineer** (full pipeline, default), **Debugger** (root-cause analysis + evidence-backed diagnosis), or **Trainer** (launch training + fix runtime errors). You can switch anytime with `/switch`.
1. **Introduces Propel** — shows all commands, skills, and agents with their triggers
2. **Drafts your CLAUDE.md** — if you have an existing codebase, scans it and generates a project-specific `.claude/CLAUDE.md` with code style, conventions, and structure already filled in. If you're starting from an empty repo, seeds a minimal CLAUDE.md that grows progressively as you work — Gate 0 answers fill in research context, first code written fills in conventions, investigations fill in domain pitfalls. Either way, you fill in the research-specific sections (research question, hypothesis, method) — these are the constraints that make Claude's output specific to your work instead of a generic average.
